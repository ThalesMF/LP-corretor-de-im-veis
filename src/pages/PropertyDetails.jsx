...
export default function PropertyDetails() {
    ...
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center px-4 text-center">
        ...
        <p className="text-brand-muted max-w-md mb-6">
          O imóvel que você está procurando não foi encontrado ou não está mais disponível no momento.
        </p>
  ...
  const relatedProperties = properties
    .filter(function(item) {
      return item.id !== property.id && (item.type === property.type || item.transactionType === property.transactionType);
    })
    .slice(0, 3);
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2.5 mb-3">
              <span className={`px-3 py-1 text-xs font-semibold uppercase tracking-wider rounded-md ${
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {property.bedrooms > 0 && (
                <div className="flex items-center gap-3 p-3 rounded-xl bg-brand-bg/60">
                  ...
                  <div>
                    <span className="block text-xs text-brand-muted">Quartos</span>
                    <span className="font-bold text-brand-dark text-base">{property.bedrooms}</span>
                  </div>
                </div>
              )}
              ...
              {property.bathrooms > 0 && (
                <div className="flex items-center gap-3 p-3 rounded-xl bg-brand-bg/60">
                  ...
                  <div>
                    <span className="block text-xs text-brand-muted">Banheiros</span>
                    <span className="font-bold text-brand-dark text-base">{property.bathrooms}</span>
                  </div>
                </div>
              )}
              ...
              {property.parkingSpaces > 0 && (
                <div className="flex items-center gap-3 p-3 rounded-xl bg-brand-bg/60">
                  ...
                  <div>
                    <span className="block text-xs text-brand-muted">Vagas</span>
                    <span className="font-bold text-brand-dark text-base">{property.parkingSpaces}</span>
                  </div>
                </div>
              )}
              ...
              {property.area && (
                <div className="flex items-center gap-3 p-3 rounded-xl bg-brand-bg/60">
                  ...
                  <div>
                    <span className="block text-xs text-brand-muted">Área Total</span>
                    <span className="font-bold text-brand-dark text-base">{property.area} m²</span>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="sticky top-28 bg-white rounded-2xl p-6 sm:p-7 border border-brand-border/80 shadow-md space-y-6">
            ...
            <div className="space-y-3">
              ...
              <p className="text-sm text-brand-muted leading-relaxed">
                Entre em contato direto com o corretor para tirar dúvidas, consultar condições de pagamento ou agendar uma visita presencial.
              </p>
            </div>
